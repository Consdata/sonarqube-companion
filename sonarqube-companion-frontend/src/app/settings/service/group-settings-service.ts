...
export class GroupSettingsService {
  ...
  saveRootGroup(group: GroupDefinition): Observable<ValidationResult> {
    return this.http
      .post<ValidationResult>(`api/v1/settings/group/`, group);
  }
  ...
  saveGroupEvent(uuid: string, event: GroupEvent): Observable<ValidationResult> {
    return this.http.post<ValidationResult>(`api/v1/settings/group/event/${uuid}`, event);
  }
