...
@Component({
  ...
  template: `
    <div class="wrapper">
      <button mat-flat-button class="dateBtn" (click)="fromPicker.open()">
        {{ range.fromString }}
      </button>
      <input
      ...
      <button mat-flat-button class="dateBtn" (click)="toPicker.open()">
        {{ range.toString }}
      </button>
      <input
...
export class TimeSelectComponent implements AfterViewInit {
  ...
  range: DateRange = {
    ...
    fromString: TimeSelectComponent.toLocalString(
      TimeSelectComponent.dateMinusDays(new Date(), 31)
    ),
    toString: TimeSelectComponent.toLocalString(
      TimeSelectComponent.dateMinusDays(new Date(), 1)
    ),
  };
  ...
  fromDateChanged($event: MatDatepickerInputEvent<Date>): void {
    this.range.from =
      $event.value || TimeSelectComponent.dateMinusDays(new Date(), 31);
  ...
  toDateChanged($event: MatDatepickerInputEvent<Date>): void {
    this.range.to =
      $event.value || TimeSelectComponent.dateMinusDays(new Date(), 1);
...
export const DEFAULT_DATE_RANGE: DateRange = {
  ...
  to: TimeSelectComponent.dateMinusDays(new Date(), 1),
  fromString: TimeSelectComponent.toLocalString(
    TimeSelectComponent.dateMinusDays(new Date(), 30)
  ),
  toString: TimeSelectComponent.toLocalString(
    TimeSelectComponent.dateMinusDays(new Date(), 1)
  ),
