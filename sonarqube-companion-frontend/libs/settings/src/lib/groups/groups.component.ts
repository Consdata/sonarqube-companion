...
import {
  GROUPS_SETTINGS_PROVIDER_TOKEN,
  GroupsSettingsProviderService,
} from './groups-settings-provider.service';
import {
  GROUPS_SETTINGS_EMITTER_TOKEN,
  GroupsSettingsEmitterService,
} from './groups-settings-emitter.service';
    ...
    <ng-container *ngIf="!(loading$ | async)">
      <ng-container *ngIf="rootGroup$ | async as vm">
        ...
        >
          ...
          >
            <ng-container
              *ngTemplateOutlet="nodeTpl; context: { node: node }"
            ></ng-container>
          </mat-tree-node>
...
export class GroupsComponent {
  ...
  constructor(
    ...
    @Inject(GROUPS_SETTINGS_PROVIDER_TOKEN)
    private provider: GroupsSettingsProviderService,
    @Inject(GROUPS_SETTINGS_EMITTER_TOKEN)
    private emitter: GroupsSettingsEmitterService
  ) {}

  hasChild = (_: number, node: GroupDetails) =>
