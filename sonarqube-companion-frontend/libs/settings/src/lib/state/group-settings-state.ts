...
export class GroupSettingsState {
  ...
  @Action(LoadGroup)
  loadGroup(ctx: StateContext<GroupConfig>, action: LoadGroup): Observable<GroupConfig> {
    return this.groupsConfigService.get(action.uuid).pipe(tap(group => ctx.setState(group)));
  }

  @Action(DeleteGroup)
  deleteGroup(ctx: StateContext<GroupConfig>, action: DeleteGroup): Observable<ValidationResult> {
