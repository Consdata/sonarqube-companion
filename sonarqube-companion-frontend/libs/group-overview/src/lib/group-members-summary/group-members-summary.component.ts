  ...
  template: `
    <ng-container *ngIf="vm$ | async as vm">
      <ng-container *ngIf="!loading">
        <sqc-violations-table
          [nameAlias]="'Member'"
          [data]="membersDiffAsViolationsTableItems(vm.summary)"
        ></sqc-violations-table>
      </ng-container>
    </ng-container>
    <ng-container *ngIf="loading">
...
export class GroupMembersSummaryComponent {
  ...
  vm$ = this.filterSubject.asObservable().pipe(
    ...
    switchMap((filter) =>
      combineLatest([
        this.groupService.groupMembersViolationsSummary(
          filter.uuid,
          filter.range
        ),
      ]).pipe(
  ...
  membersDiffAsViolationsTableItems(
    projectsHistory: MemberViolationsSummary[]
  ): ViolationsTableItem[] {
